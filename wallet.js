    ...
    if (sendBtn) {
        sendBtn.addEventListener('click', () => {
            ...
            if (balance === 0) {
                ...
                let depDateStr = currentUserProfile?.first_deposit_date;
                let depDate = depDateStr ? new Date(depDateStr) : new Date();
                depDate.setMonth(depDate.getMonth() + 1);
                const options = { year: 'numeric', month: 'short', day: 'numeric' };
                showToast(`You can withdraw your actual balance at ${depDate.toLocaleDateString(undefined, options)}`);
            }
        });
    }
...
window.submitWithdrawProfit = async function() {
    ...
    if (isNaN(amountUsd) || amountUsd < 0.0001) {
        showToast('Minimum withdrawal is $0.0001', 'error');
        return;
    }
